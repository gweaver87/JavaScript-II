...
// Problem 1 list showing how much each company donated
runners.forEach(function(element){
    console.log(`${element.company_name} ${element.donation}`)
});

// Problem 2 list of elite, donations 250 +
let eliteDonators = [];
eliteDonators = runners.filter(function (item) {
  return item.donation >= 250;}).map(function(item) {
  return `${item.first_name} ${item.last_name} : ${item.email}`
})
console.log(eliteDonators);
